...
def main():
    ...
    MODEL_PATH = "/home/surendar/Drone_pro/ros2_arm_ws/src/human_pose_det/human_pose_det/Move_net.tflite"

    rclpy.init()
    pub = GesturePublisher()
    ...
    _, in_h, in_w, in_c = input_info[0]["shape"]
    in_dtype = input_info[0]["dtype"]

    cam = CameraThread(0)

    # -------------------------------------------------
    # Thresholds for stable 3-zone detection
    # -------------------------------------------------
    UPPER_ZONE = 0.20     # must go 20% above shoulder
    LOWER_ZONE = 0.20     # must go 20% below shoulder
    MIN_CONF   = 0.35     # minimum confidence

    fps_list = deque(maxlen=30)
    last_t = time.time()

    print("\n🤖 3-ZONE POSE CONTROL (Left=Up/Down, Right=Pick/Release)\n")

    try:
        while rclpy.ok():

            # ROS2 non-blocking spin
            rclpy.spin_once(pub, timeout_sec=0.0)
            ...
            interpreter.set_tensor(input_info[0]["index"], inp)
            interpreter.invoke()

            kp = interpreter.get_tensor(output_info[0]["index"]).reshape(-1, 3)
            ...
            if lw_s > MIN_CONF and ls_s > MIN_CONF:
                if lw_y < ls_y - UPPER_ZONE:
                    up = True
                elif lw_y > ls_y + LOWER_ZONE:
                    down = True
                # else → neutral → both False

            # ----------------------------------------------
            # RIGHT HAND → PICK / RELEASE
            # ----------------------------------------------
            pick = False
            release = False

            if rw_s > MIN_CONF and rs_s > MIN_CONF:
                if rw_y < rs_y - UPPER_ZONE:
                    pick = True
                elif rw_y > rs_y + LOWER_ZONE:
                    release = True
                # else → neutral → both False
            ...
            pub.send_down(down)
            pub.send_pick(pick)
            pub.send_release(release)

            print(
                f"L_UP={up}  L_DOWN={down} | R_PICK={pick}  R_RELEASE={release}",
                end="\r"
            )
